    ...
    const D3D12_RANGE* pWrittenRange)
{
    ...
    DEBUG_ASSERT(mMappedReferenceCount >= 0);
}
...
D3D12_GPU_VIRTUAL_ADDRESS STDMETHODCALLTYPE CD3DX12AffinityResource::GetGPUVirtualAddress(void)
{
    ...
    {
        std::vector<D3D12_GPU_VIRTUAL_ADDRESS> Addresses(D3DXAFFINITY_GET_NODE_COUNT);
        for (UINT i = 0; i < D3DX12AFFINITY_MAX_ACTIVE_NODES;i++)
        {
            if (((1 << i) & mAffinityMask) != 0)
            {
...
void CD3DX12AffinityResource::SynchronizeAcrossDevices()
{
    ...
    case D3D12_CPU_PAGE_PROPERTY_UNKNOWN:
        ...
        break;
    default:
        D3DX12AFFINITY_LOG("Unexpected CPU page value: %d\n", mCPUPageProperty);
