...
private:
	...
	Internal_CD3DX12AffinityDevice* mInternal = nullptr;
public:	
	void DbgValidateDescriptorSet(D3D12_CPU_DESCRIPTOR_HANDLE desc, CD3DX12AffinityResource* res);
	CD3DX12AffinityResource* DbgValidateDescriptorGet(D3D12_CPU_DESCRIPTOR_HANDLE desc);

    void WriteApplicationMessage(D3D12_MESSAGE_SEVERITY const Severity, char const* const Message);
};

struct D3DX12AF_API CD3DX12AffinityDeviceStatics
{
	...
	static UINT g_CachedActualNodeCount;
	static UINT g_CachedActualNodeMask;


	static UINT GetActiveNodeID();
	static UINT GetActiveNodeID_RAW();
	static void SetActiveNodeID(UINT id);
};

class CD3DX12AffinityGPUNodeIDScope
{
public:
	CD3DX12AffinityGPUNodeIDScope(UINT id)
	{
		mPrevNodeIndex = CD3DX12AffinityDeviceStatics::GetActiveNodeID_RAW(); 
		CD3DX12AffinityDeviceStatics::SetActiveNodeID(id);
	}
	~CD3DX12AffinityGPUNodeIDScope()
	{
		CD3DX12AffinityDeviceStatics::SetActiveNodeID(mPrevNodeIndex);
	}
protected:
	UINT mPrevNodeIndex;
};