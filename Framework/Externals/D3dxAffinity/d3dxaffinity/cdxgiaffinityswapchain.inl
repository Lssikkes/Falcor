    ...
    UINT AffinityMask)
{
    ...
    {
        UINT const i = 0;
        D3DX12AFFINITY_LOG("Presenting contents of backbuffer %d on device %d", mCurrentBackBufferIndex, i);

        D3DX12AFFINITY_LOG(" [marshalled]\n");
    ...
    UINT AffinityMask)
{
    ...
    case EAffinitySwapChainMode::LDA:
    {
        for (UINT b = 0; b < mNumBackBuffers; ++b)
        {
            ...
            {
                UINT nodeIndex = b % D3DXAFFINITY_GET_NODE_COUNT;
...
HRESULT STDMETHODCALLTYPE CDXGIAffinitySwapChain::ResizeBuffers(
    ...
    UINT AffinityMask)
{
    D3DX12AFFINITY_LOG("Doing swap chain buffer resize...\n");
    UINT EffectiveAffinityMask = (AffinityMask == 0) ? GetNodeMask() : AffinityMask & GetNodeMask();
    ...
    case EAffinitySwapChainMode::LDA:
    {
        ...
        UINT nodeCount = D3DXAFFINITY_GET_NODE_COUNT;
        if (nodeCount > 1)
        {
			...
			D3DX12AFFINITY_LOG("CD3DX12AffinityGraphicsCommandList::ResizeBuffers (active node update) %d %llX", GetParentDevice()->GetActiveNodeIndex(), (unsigned long long)this);
#endif
        }
        else
        {
    ...
    UINT const NumBackBuffers)
{
    for (UINT b = 0; b < NumBackBuffers; ++b)
    {
        ...
        {
            renderTargetHeapProps.CreationNodeMask = AffinityDevice->AffinityIndexToNodeMask(Index)  & CD3DX12AffinityDeviceStatics::g_CachedActualNodeMask;
            renderTargetHeapProps.VisibleNodeMask = AffinityDevice->LDAAllNodeMasks()  & CD3DX12AffinityDeviceStatics::g_CachedActualNodeMask;
        }
        ...
        if (AffinityDevice->GetAffinityMode() == EAffinityMode::LDA)
        {
            heapDesc.Properties.CreationNodeMask = AffinityDevice->AffinityIndexToNodeMask(Index) & CD3DX12AffinityDeviceStatics::g_CachedActualNodeMask;
            heapDesc.Properties.VisibleNodeMask = AffinityDevice->LDAAllNodeMasks()  & CD3DX12AffinityDeviceStatics::g_CachedActualNodeMask;
        }
