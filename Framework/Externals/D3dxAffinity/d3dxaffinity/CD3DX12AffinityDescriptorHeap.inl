...
{
    UINT const NodeCount = D3DXAFFINITY_GET_NODE_COUNT;
    ...
    D3DX12AFFINITY_LOG("Used up to index %u in heap array\n", maxindex);

    D3DX12AFFINITY_LOG("Created a descriptor heap with CPU start at 0x%IX and GPU start a 0x%IX\n", (size_t)mCPUHeapStart, (size_t)mGPUHeapStart);
    for (UINT i = 0; i < NodeCount; ++i)
    {
        D3DX12AFFINITY_LOG("  Device %u CPU starts at 0x%IX and GPU starts at 0x%IX\n",
...
{
    if (mCPUHeapStart != nullptr)			delete[] mCPUHeapStart;
    if (mGPUHeapStart != nullptr)			delete[] mGPUHeapStart;
}
