...
HRESULT STDMETHODCALLTYPE CD3DX12AffinityObject::QueryInterface(
	REFIID riid,
	...
	UINT AffinityMask)
{
	...
	for (int AffinityIndex = 0; AffinityIndex < D3DX12AFFINITY_MAX_ACTIVE_NODES; AffinityIndex++)
	{
...
{
	for (size_t i = 0; i < D3DX12AFFINITY_MAX_ACTIVE_NODES; ++i)
	{
		if (mObjects[i])
		{
			ULONG RefCount = mObjects[i]->Release();
			D3DX12AFFINITY_LOG("Releasing handle 0x%p, ref count is now %lu\n", mObjects[i], RefCount);
		}
	}
...
#ifdef D3DXAFFINITY_DEBUG_OBJECT_NAME
	D3DX12AFFINITY_LOG("Release() called on an affinity object, reference count now %u [%p] [%s] [%s]\n", mReferenceCount, this, mObjectTypeName.c_str(), mObjectDebugName.c_str());
#endif
...
{
	return CD3DX12AffinityDeviceStatics::g_CachedNodeMask;
}
...
{
	return CD3DX12AffinityDeviceStatics::GetActiveNodeID();
}
